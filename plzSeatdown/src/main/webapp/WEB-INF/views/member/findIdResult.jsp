...
<link rel="stylesheet" href="${contextPath}/resources/css/member-findIdPwdResult.css" />
</head>
<body class="homepage is-preload">
	...
	<div class="container findContainer">
		<div class="row">
			...
			<div class="col-md-10">
				<div id="wrapper">
					<h4><i id="infoIcon" class="fas fa-info-circle"></i> 아이디 찾기</h4>
					<hr>
					<ul class="infomation">
						<li><p>SNS계정(네이버, 카카오, 구글)회원은 아이디/비밀번호 찾기가 제공되지 않습니다.</p></li>
					...
					<div class="tab-container">
						<div id="tab1" class="tab-content">
							<form class="form-horizontal" role="form" method="POST" action="sendId">
								<p>회원님의 아이디는 <span style="font-weight: bold">${maskId}</span> 입니다.</p>
								<p style="font-size: 0.8em; color: red;">※ 해당 정보는 개인정보보호 가이드라인 지침에 따라 마스킹 처리 되었습니다. 
								전체 아이디는 메일을 통해 확인할 수 있습니다.</p>
								<input type="hidden" name="memberId" value="${memberId}">
								<input type="hidden" name="memberEmail" value="${memberEmail}">
								<div class="col-md-4 findBox">
			...
			$("#selectEmail2").change(function(){
				$("#selectEmail2 option:selected").each(function(){
					if($(this).val() == "1"){ // 직접 입력일 경우
						$("#email4").val(''); // 값 초기화
						$("#email4").prop("disabled", false); // 입력창 활성화
					}else{ // 직접 입력이 아닐 경우
						$("#email4").val($(this).text()); // 선택값 입력
						$("#email4").attr("disabled", true); // 입력창 비활성화
					}
				});
			});
