...
public class MypageServiceImpl implements MypageService {
	...
	public int updateMypage(Member member, MultipartFile image, String savePath, int deleteCheck) throws Exception {
		...
		if (!image.getOriginalFilename().equals("")) {
			...
			if (file != null) { // 이미지가 비어있지 않다면
				updateFile = new Profile(file.getProfileNo(), changeFileName);
			}
			// 이미지 기존에 x -> insert 진행
			else {
		...
		if (result > 0 && deleteCheck == 1) {
			...
			if (result == 0)
				throw new Exception("파일 삭제 과정에서 오류 발생");
		}
		...
		if (file != null) {
			if (file.getProfileNo() == insertFile.getProfileNo()) {
				File deletefile = new File(savePath + "/" + file.getProfilePath());
				deletefile.delete();
			}
	...
	public int getWriteReply(int memberNo) throws Exception {
		return mypageDAO.getWriteReply(memberNo);
	}
	...
	public int nicknameDupCheck(String memberNickname) throws Exception {
		return mypageDAO.nicknameDupCheck(memberNickname);
	}
